...
function SideNav({params}) {
    ...
    const [renamingDoc, setRenamingDoc] = useState(null);  // Track the document being renamed
    const [newDocName, setNewDocName] = useState('');  // Track the new name input
    ...
    const handleRename = async (docId) => {
        if (!newDocName.trim()) return;  // Do nothing if the name is empty

        const docRef = doc(db, 'workspaceDocuments', docId.toString());
    ...
    return (
        <div className='h-screen md:w-72 hidden md:block fixed bg-blue-50 p-5 shadow-md'>
            ...
            {/* Document List inside a Scrollable Box with Styled Scrollbar */}
            <div className="mt-4 max-h-[400px] overflow-y-auto scrollbar-thin scrollbar-thumb-[rgba(130,48,255,0.2)] scrollbar-track-transparent scrollbar-thumb-rounded-lg">
                <DocumentList 
            ...
            <div className='absolute bottom-10 w-[85%]'>
                ...
                <h2 className='text-sm font-light my-2'>
                    <strong>{documentList?.length}</strong> Out of <strong>15</strong> files used
                </h2>
